...
`asap` mode aggressively queues the next iteration as soon as the prior send is accepted. In `terminal` mode it does not wait for the bound Terminal tab to become idle; it sends the prompt, waits briefly, then requires one observed busy/working state before scheduling the next ASAP fire after a short queue throttle. If no turn-start/busy state is observed, pause the job. The default queue throttle is 30 seconds and can be overridden with `CODEX_LOOP_TERMINAL_ASAP_QUEUE_DELAY`.
...
- Schedule modes:
  ...
  - `asap`: explicit `asap` input such as `asap keep going`; the next run is queued after the bound target tab accepts Return and shows a busy/working state, plus a small queue throttle. It intentionally does not wait for idle before sending the next prompt. A failed turn-start observation pauses the job as a safety stop.
